...
public class Protocol {
    ...
    public void dataProcess(boolean isdebug) {
        stateProcess();
        dataReceiveProcess(isdebug);
//        dataSendProcess();
    }
    ...
    private int makeData(SwitchData sdata, long cmdpwd) {
        int ptr = 0;
        ...
        //todo ?
        link.sendData[ptr++] = (byte) (cmdpwd & 0xff);
        link.sendData[ptr++] = (byte) ((cmdpwd >> 8) & 0xff);
        link.sendData[ptr++] = (byte) ((cmdpwd >> 16) & 0xff);
        link.sendData[ptr++] = (byte) ((cmdpwd >> 24) & 0xff);
        link.sendData[ptr++] = (byte) ((cmdpwd >> 32) & 0xff);
        link.sendData[ptr++] = (byte) ((cmdpwd >> 40) & 0xff);
    ...
    private void dataReceiveProcess(boolean isdebug) {
        ...
        while (sum >= 12) {
            if (link.receiveData[ptr] == 0x68 && link.receiveData[ptr + 7] == 0x68 && link.receiveData[ptr + 9] != 0) {
                ...
                if (frame_len > sum)
                    break;
                if ((isdebug == false) && calcSum(ptr, frame_len - 2, true) != link.receiveData[ptr + frame_len - 2]) {
                    break;
                }
...
//                if (getLongData(ptr - 9, 6) == getLongData(ptr, 6)) {
                ...
                if (dataFlag) {
                    ...
                    result.Ib = data.Ib;
                    result.Ic = data.Ic;
                    result.imbalance = data.imbalance;
                    result.name = data.name;
                    result.num = data.num;
                    result.sdata = new ArrayList<SwitchData>();
                    result.sdata.addAll(data.sdata);
    ...
    public boolean setSwitchData(SwitchData sdata, long pwd) {
        if (sdata == null) {
            Log.w(TAG, "sdata is null");
            return false;
        }
        sendNum = makeData(sdata, pwd);
        return true;
    }
